...
def load_rating(dataset):
    ...
    """
    file_path = f'./data/{dataset}.csv'
    return pd.read_csv(file_path)[:1000].values
...
def gen_graph(rating_data, n_users, n_items):
    """
...
def load_data(dataset):
    df, n_users, n_items = split_rating_data(dataset)
    rating_data = load_rating(dataset)
    # trust_data = load_trust_network(dataset)
    u, v = get_nodes(dataset)
    G = gen_graph(rating_data, n_users, n_items)  # Full graph
    ...
    return adj, features, df, G, n_users
